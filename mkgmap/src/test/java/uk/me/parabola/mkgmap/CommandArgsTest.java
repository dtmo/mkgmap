...
public class CommandArgsTest {
	...
	public void testDefaultMapnames() {
		...
		assertEquals("63240001", proc.getProperty(0, "mapname"), "first file");
		assertEquals("63240002", proc.getProperty(1, "mapname"), "second file");
	}
	...
	public void testNumericNames() {
		...
		assertEquals("00000001", proc.getProperty(0, "mapname"), "first numeric name");
		assertEquals("00000002", proc.getProperty(1, "mapname"), "first numeric name");
	}
	...
	public void testMapnameWithNumericFilenames() {
		...
		assertEquals(FILE1, arg.name, "file name");
		assertEquals(SETNAME1, arg.getProperty("mapname"), "first file");

		arg = proc.getFileArg(1);
		assertEquals(FILE2, arg.name, "file name");
		assertEquals(SETNAME2, arg.getProperty("mapname"), "second file");
	...
	public void testArgumentFile() throws IOException {
		...
		ArgCollector.FileArgs arg = proc.getFileArg(0);
		assertEquals(F1, arg.name, "file name");
		assertEquals(SETNAME1, arg.getMapname(), "first file");
		assertEquals("VIC", arg.getProperty("region-abbr"), "region-abbr");

		arg = proc.getFileArg(1);
		assertEquals(F2, arg.name, "file name");
		assertEquals(SETNAME2, arg.getMapname(), "second file");
		assertEquals("NSW", arg.getProperty("region-abbr"), "region-abbr");
		assertEquals("OSM-AU New South Wales", arg.getProperty("description"), "description");
	}
	...
	public void testComplexMapname() {
		...
		assertEquals("63240001", proc.getProperty(0, "mapname"), "just default");
		assertEquals("00000001", proc.getProperty(1, "mapname"), "numeric");
		assertEquals(SETNAME, proc.getProperty(2, "mapname"), "with mapname");
		assertEquals("12345679", proc.getProperty(3, "mapname"), "continue after set");
		assertEquals("12345680", proc.getProperty(4, "mapname"), "continue after set");
	}
	...
	public void testArgReset() {
		...
		assertEquals("", proc.getProperty(0, "keep-going"), "first file has keep-going");
		assertEquals(null, proc.getProperty(1, "keep-going"), "second file does not have keep-going");
		assertEquals("", proc.getProperty(2, "keep-going"), "third file does has keep-going");
		assertEquals(null, proc.getProperty(1, "invalid-option-does-not-exist"), "any option that was not present is null");
	}
