...
public class ValueBuilderTest {
	...
	public void testMultiSubst() {
		...
		String s = vb.build(el, null);
		assertEquals("unset", s, "name not set, so default is applied");

		// Name tag is set, so substitutions are made
		el.addTag("name", "abc def");
		s = vb.build(el, null);
		assertEquals("xyz www", s, "substitutions in name");
	}
	...
	public void testQuotedArgs() {
		...
		// No tags set, so default value will be applied.
		String s = vb.build(el, null);
		assertEquals("unset string", s, "name not set, so default is applied");

		// Name tag is set, so substitutions are made
		el.addTag("name", "abc defg");
		s = vb.build(el, null);
		assertEquals("x|y w|w\"w", s, "substitutions in name");
	}
	...
	public void testSpacedQuotedArgs() {
		...
		el.addTag("name", "abc defg");
		s = vb.build(el, null);
		assertEquals("x|y w|w", s, "substitutions in name");
	}
	...
	public void testSpacedArgsOldSyntax() {
		...
		String s = vb.build(el, null);
		assertEquals("{ name '2345678901234' }", s, "value is trimmed");
	}
